...
const StackProvider = ({
  ...
  const updateStacks = useCallback(
    (to: string | number, isClear = false) => {
      ...
      if (isToNo) {
        ...
        inMemoryCache.setScreens(
          isClear ? [stackData] : [...baseStack, stackData]
        );
  ...
  const checkGoForward = () => {
    ...
    const index =
      typeof stateIndex === "number" ? stateIndex : historyIndex + 1;
  ...
  const initStorageStackData = useCallback(() => {
    ...
    const storageStacksData = JSON.parse(
      window.sessionStorage.getItem(STORAGE_KEY_SCREEN_STACKS)
    );
  ...
  const animationDuration =
    typeof duration === "number" ? duration : ANIMATION_DURATION;
